        ...
        <section key={b.category} className="card stack-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="progress-track">
              <div className="h-full rounded-full" style={{ width: `${(b.spent / b.maximum) * 100}%`, backgroundColor: b.color.cssVar }} />
            </div>
            <div className="row-between">
              <div className="text-preset-4 text-grey-500">Spent</div>
              <div className="text-preset-4 text-grey-900">{toCurrency(b.spent)}</div>
            </div>
          </div>
          ...
          <div className="row-between">
            ...
            <div className="text-preset-4 text-grey-900">{toCurrency(b.free)}</div>
          </div>
          ...
          <div className="row-between">
            ...
            <button className="text-preset-5-bold text-grey-500">See All</button>
          </div>
            ...
            {b.latest.map((t, idx) => (
              <li key={`${t.name}-${idx}`} className="flex items-center justify-between py-3">
                <div className="flex items-center gap-3">
                  {t.avatar ? <img className="h-6 w-6 rounded-full" src={t.avatar} alt="" /> : <div className="h-6 w-6 rounded-full bg-grey-100" />}
                  <span className="text-preset-4 text-grey-900">{t.name}</span>
                </div>
                <div className="text-preset-4 text-grey-900">-{toCurrency(Math.abs(t.amount))}</div>
              </li>
            ))}
          </ul>
        </section>
