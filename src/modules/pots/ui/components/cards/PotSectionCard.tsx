...
export const PotSectionCard = ({ item, index, onEdit, onDelete, onAddMoney, onWithdraw }: { item: PotItem; index: number; onEdit: (index: number) => void; onDelete: (index: number) => void; onAddMoney: (index: number) => void; onWithdraw: (index: number) => void }) => {
  ...
  return (
    <section className="pot-card">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button className="btn-soft" onClick={() => onAddMoney(index)}>+ Add Money</button>
        <button className="btn-soft" onClick={() => onWithdraw(index)}>Withdraw</button>
      </div>
    </section>
  );
};